 ...
 * @author Maytopacka
 */
public class Dlg_waiters extends javax.swing.JDialog {
...
//</editor-fold>
    ...
    private Dlg_waiters(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        setUndecorated(true);
        initComponents();
        myInit();
    ...
    private Dlg_waiters(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        setUndecorated(true);
        initComponents();
        myInit();
    ...
    private Dlg_waiters myRef;

    private void setThisRef(Dlg_waiters myRef) {
        this.myRef = myRef;
    }
    private static java.util.Map<Object, Dlg_waiters> dialogContainer = new java.util.HashMap();
    ...
    public static Dlg_waiters create(java.awt.Window parent, boolean modal) {
    ...
    public static Dlg_waiters create(java.awt.Window parent, java.awt.Dialog.ModalityType modalType) {

        if (parent instanceof java.awt.Frame) {

            Dlg_waiters dialog = dialogContainer.get(parent);

            if (dialog == null) {
                dialog = new Dlg_waiters((java.awt.Frame) parent, false);
        ...
        if (parent instanceof java.awt.Dialog) {
            ...
            if (dialog == null) {
                dialog = new Dlg_waiters((java.awt.Dialog) parent, false);
                dialog.setModalityType(modalType);
                dialogContainer.put(parent, dialog);
                java.util.logging.Logger.getAnonymousLogger().
                        log(Level.INFO, "instances: {0}", dialogContainer.size());
    ...
    public static void main(String args[]) {
        ...
        Dlg_waiters dialog = Dlg_waiters.create(new javax.swing.JFrame(), true);
    ...
    private void initComponents() {
        ...
        tbl_waiters = new org.jdesktop.swingx.JXTable();
        ...
        tf_waiter_name = new javax.swing.JTextField();
        ...
        tf_waiter_name.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
        tf_waiter_name.setHorizontalAlignment(javax.swing.JTextField.LEFT);
    ...
    private org.jdesktop.swingx.JXTable tbl_waiters;
    private javax.swing.JTextField tf_search;
    private javax.swing.JTextField tf_waiter_name;
    ...
    private void focus() {
        JTextField[] tf = {tf_search, tf_waiter_name};
...
//                btn_0.doClick();
    ...
    private void init_tbl_waiters() {
        ...
        tbl_waiters.getTableHeader().setFont(new java.awt.Font("Arial", 0, 11));
        tbl_waiters.setRowHeight(35);
        tbl_waiters.setFont(new java.awt.Font("Arial", 0, 11));
    }
    ...
    private void data_cols() {
        String search = "";
        loadData_waiters(S1_waiters.ret_data(search));
    }

    private void add_waiters() {
        int id = -1;
        String waiter_name = tf_waiter_name.getText();
        to_waiters to = new to_waiters(id, waiter_name);
        S1_waiters.add_waiters(to);
        data_cols();
        clear_waiters();
        Alert.set(1, "");
    }

    private void select_waiters() {
        int row = tbl_waiters.getSelectedRow();
        if (row < 0) {
            return;
        }
        to_waiters to = (to_waiters) tbl_waiters_ALM.get(tbl_waiters.convertRowIndexToModel(row));
        tf_waiter_name.setText(to.waiter_name);
    }

    private void edit_waiters() {
        int row = tbl_waiters.getSelectedRow();
        if (row < 0) {
            return;
        }
        ...
        int id = to.id;
        String waiter_name = tf_waiter_name.getText();
        to_waiters to1 = new to_waiters(id, waiter_name);
        S1_waiters.edit_waiters(to1);
        data_cols();
        clear_waiters();
        Alert.set(2, "");
    }

    private void clear_waiters() {
        tf_waiter_name.setText("");
    }

    private void delete_waiters() {
        int row = tbl_waiters.getSelectedRow();
        if (row < 0) {
            return;
        }
        to_waiters to = (to_waiters) tbl_waiters_ALM.get(tbl_waiters.convertRowIndexToModel(row));
        S1_waiters.delete_waiters(to);
        data_cols();
        clear_waiters();
        Alert.set(3, "");
    }
