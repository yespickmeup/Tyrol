...
import POS.printing_assembly.S1_printing_assembly.to_printing_assembly;
...
//</editor-fold>
    ...
    private void initComponents() {
        ...
        tbl_printing_assembly = new org.jdesktop.swingx.JXTable();
    ...
    private org.jdesktop.swingx.JXTable tbl_printing_assembly;
    ...
    private void focus() {
        JTextField[] tf = {tf_search,tf_location};
...
//                btn_0.doClick();
    ...
    private void init_tbl_printing_assembly() {
        ...
        int[] tbl_widths_printing_assembly = {100, 0};
        ...
        tbl_printing_assembly.getTableHeader().setFont(new java.awt.Font("Arial", 0, 11));
        tbl_printing_assembly.setRowHeight(35);
        tbl_printing_assembly.setFont(new java.awt.Font("Arial", 0, 11));
    }
    ...
    private void data_cols() {
        String search = "";
        loadData_printing_assembly(S1_printing_assembly.ret_data(search));
    }

    private void add_printing_assembly() {
        int id = -1;
        String location = tf_location.getText();
        to_printing_assembly to = new to_printing_assembly(id, location);
        S1_printing_assembly.add_printing_assembly(to);
        data_cols();
        clear_printing_assembly();
        Alert.set(1, "");
    }

    private void select_printing_assembly() {
        int row = tbl_printing_assembly.getSelectedRow();
        if (row < 0) {
            return;
        }
        to_printing_assembly to = (to_printing_assembly) tbl_printing_assembly_ALM.get(tbl_printing_assembly.convertRowIndexToModel(row));
        tf_location.setText(to.location);
    }

    private void edit_printing_assembly() {
        ...
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                to_printing_assembly to1 = new to_printing_assembly(id, location);
                S1_printing_assembly.edit_printing_assembly(to1);
                data_cols();
                clear_printing_assembly();
                Alert.set(2, "");
            }
    ...
    private void clear_printing_assembly() {
        tf_location.setText("");
    }

    private void delete_printing_assembly() {
        int row = tbl_printing_assembly.getSelectedRow();
        if (row < 0) {
            return;
        }
        to_printing_assembly to = (to_printing_assembly) tbl_printing_assembly_ALM.get(tbl_printing_assembly.convertRowIndexToModel(row));
        S1_printing_assembly.delete_printing_assembly(to);
        data_cols();
        clear_printing_assembly();
        Alert.set(3, "");
    }
